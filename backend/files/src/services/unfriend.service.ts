...
export const unfriendUser = async (req: FastifyRequest<{ Params: { id: number } }>, res: FastifyReply) => {
  try {
    ...
    if (removedFriend.changes == 0 || removed.changes == 0) return res.status(404).send({ error: "Error" });
    res.status(200).send();
  } catch (err) {
