...
export const editUserInfos = async (req, res) => {
    try {
        ...
        user = app.db.prepare("SELECT * from players WHERE username = ? AND id <> ?").get(username, payload?.id);
        if (user)
            return res.status(401).send({ error: "Username already exist" });
        //check if user email already exists
        user = app.db.prepare("SELECT * from players WHERE email = ? AND id <> ?").get(email, payload?.id);
        if (user)
