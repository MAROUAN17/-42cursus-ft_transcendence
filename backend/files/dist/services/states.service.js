...
export const get_player_rooms = async (req, res) => {
    try {
        const playerId = Number(req.params?.playerId);
        if (!playerId) {
            return res.status(400).send({ error: "Missing player id" });
...
export const get_leaderboard_dashboard = async (req, res) => {
    try {
        const stmt = app.db.prepare(`
      SELECT 
        p.id,
...
export const get_player_week_activity = async (req, res) => {
    try {
        const playerId = Number(req.params?.playerId);
        if (!playerId) {
            return res.status(400).send({ error: "Missing player id" });
    ...
    `);
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const found = dailyMatches.find((m) => m.day === dayStr);
            last7Days.push({
                day: dayStr,
