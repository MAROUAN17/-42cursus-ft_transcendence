...
export const checkResetPass = async (req, res) => {
    try {
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (user && user.intra_id)
            return res.status(401).send({ error: "Email is linked to intra42 account" });
...
export const verifyResetPin = async (req, res) => {
    try {
        ...
        if (!password || !confirmPassword)
            return res.status(401).send({ error: "Password or confirm password should be provided" });
