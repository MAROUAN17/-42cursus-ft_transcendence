...
export const start_games = async (req, res) => {
...
export const create_tournament = async (req, res) => {
    try {
        ...
        VALUES (?, ?, ?, ?, ?)
    `);
        const info = stmt.run(tournament.name, JSON.stringify(tournament.players), tournament.createdAt.toISOString(), tournament.status, tournament.admin);
...
export const join_tournament = async (req, res) => {
    const { playerId } = req.body;
...
export const get_tournaments = async (req, res) => {
    try {
        const tournaments = app.db.prepare("SELECT * FROM Tournament").all();
        const resData = tournaments.map((t) => {
...
export const get_tournament_by_id = async (req, res) => {
    try {
        const tournamentId = Number(req.params?.tournamentId);
        if (!tournamentId) {
        ...
        const playerIds = JSON.parse(tournament.players);
...
export const delete_tournament = async (req, res) => {
    const tournamentId = Number(req.body?.tournamentId);
    if (!tournamentId)
        return res.status(400).send({ error: "Missing tournamentId" });
    try {
...
export const leave_tournament = async (req, res) => {
    try {
        ...
        if (!tournament)
            return res.status(404).send({ error: "Tournament not found" });
        let players = JSON.parse(tournament.players);
        if (!players.includes(playerId))
...
export const start_tournament = async (req, res) => {
    const { playerId } = req.body;
    const tournamentId = Number(req.params?.tournamentId);
    if (!playerId || !tournamentId)
        return res.status(400).send({ error: "Missing player info" });
    ...
    if (Number(tournament.admin) != playerId)
        return res.status(403).send({ error: "Only admin can start the tournament" });
    if (players.length < 4)
  ...
  `);
    const insertMany = app.db.transaction((rounds) => {
        for (const round of rounds)
            insertRound.run(round.tournament_id, round.player1, round.player2, round.round_number);
    });
    insertMany(rounds);
    return res.send({ success: true, msg: "Tournament started", rounds });
};
export const get_rounds = async (req, res) => {
    try {
        const tournamentId = Number(req.params?.tournamentId);
        if (!tournamentId) {
            return res.status(400).send({ error: "Missing tournamentId" });
...
export const get_score = async (req, res) => {
    // console.log("--- entered");
    try {
...
export const report_match_result = async (req, res) => {
    const { playerId } = req.body;
    const { roundId, score1, score2 } = req.body;
    ...
    if (score1 > score2)
        ...
        return res.status(400).send({ error: "Match cannot end in a tie (ta3adol)" });
...
export const get_tournament_winner = async (req, res) => {
    try {
        const tournamentId = Number(req.params?.tournamentId);
        if (!tournamentId) {
            return res.status(400).send({ error: "Missing tournamentId" });
...
export const get_final_round = async (req, res) => {
    try {
        const tournamentId = Number(req.params?.tournamentId);
        if (!tournamentId) {
