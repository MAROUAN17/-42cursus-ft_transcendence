...
export const updateCustomization = async (req, res) => {
    try {
        ...
        const updatedRow = app.db
            .prepare("UPDATE settings SET gameBorder = ?, gameShadow = ?, ballColor = ?, ballShadow = ?, paddleColor = ?, paddleBorder = ?, paddleShadow = ?, paddleSpeed = ?, selectedBg = ? WHERE userId = ?")
...
export const getCustomization = async (req, res) => {
    try {
        ...
        const payload = app.jwt.jwt1.verify(token);
        const row = app.db
            .prepare("SELECT gameBorder, gameShadow, ballColor, ballShadow, paddleColor, paddleBorder, paddleShadow, paddleSpeed, selectedBg FROM settings WHERE userId = ?")
            .get(payload.id);
...
//# sourceMappingURL=updateCustomization.service.js.map