import { DefaultGame } from "../models/game.js";
...
const rooms = [];
// const rounds:Round[] = [];
function set_random_Info(game_info) {
...
function saveData(room) {
    ...
    if (room.tournamentId) {
        try {
            ...
            if (room.round == 2)
                app.db.prepare("UPDATE tournament SET status = ? WHERE id = ?").run("finished", room.tournamentId);
...
export const getData = async (req, res) => {
    try {
        const history_rooms = app.db.prepare("SELECT * FROM Room").all();
...
function getRoom(gameId, roundId) {
    var room;
    if (roundId)
...
function addPlayerToRoom(gameId, playerId, side) {
    if (!playerId)
        return;
...
function wait_opponent(room, time, opponent) {
    if (!room.waitTimer) {
...
function addPlayerToRound(tournamentId, playerId, rn, side) {
    if (!playerId)
        return;
...
const check_existing = (playerId) => {
    for (let i = 0; i < activeGames.length; i++) {
        ...
        if (!rooms[i]?.ready)
            continue;
        if (rooms[i]?.player1 == playerId || rooms[i]?.player2 == playerId)
            return true;
    }
    return false;
};
