...
function sendNotification(currPacket) {
    if (currPacket.type == "onlineStatus" || currPacket.type == "logNotif" || currPacket.type == "gameEvent" || currPacket.type == "NotifyChange")
        return;
    const notif = app.db
...
function checkNotificationMssg(currPacket) {
    if (currPacket.type == "onlineStatus" || currPacket.type == "logNotif" || currPacket.type == "gameEvent" || currPacket.type == "NotifyChange")
        return;
    const updatedRow = app.db
        .prepare("UPDATE notifications SET isRead = true, unreadCount = unreadCount + 1, updatedAt = (datetime('now')), message = ? WHERE sender_id = ? AND recipient_id = ? AND type = ?")
...
function checkNotificationFriend(currPacket) {
    if (currPacket.type == "onlineStatus" || currPacket.type == "logNotif" || currPacket.type == "gameEvent" || currPacket.type == "NotifyChange")
        return;
    const notif = app.db
        .prepare("SELECT * FROM notifications WHERE sender_id = ? AND recipient_id = ? AND type = ?")
...
function handleBlock(currPacket) {
    if (currPacket.type == "onlineStatus" || currPacket.type == "logNotif" || currPacket.type == "gameEvent" || currPacket.type == "NotifyChange")
        return;
    let client = clients.get(currPacket.data.recipient_id);
    if (client) {
...
function handleMessage(currPacket) {
    const updatedRow = app.db
        .prepare("UPDATE notifications SET isRead = true, unreadCount = unreadCount + 1, updatedAt = (datetime('now')), message = ? WHERE sender_id = ? AND recipient_id = ? AND type = ?")
        .run(currPacket.data.message, currPacket.data.sender_id, currPacket.data.recipient_id, "message");
...
function handleMsgMarkSeen(currPacket) {
    app.db
...
function handleNotifMarkSeen(currPacket) {
    app.db
...
function handleLogNotif(packet) {
...
function handleGameInvite(packet) {
...
function handleGameInviteRes(packet) {
...
function notifyMembersChange(packet) {
    const tournament = app.db.prepare("SELECT players FROM tournament WHERE id = ?").get(packet.data.tournamentId);
    if (!tournament)
        return;
...
function notifyTournamentStart(packet) {
...
async function processMessages() {
    ...
    while (messageQueue.length > 0) {
        const currPacket = messageQueue.shift();
        if (!currPacket)
            return;
        try {
            if (currPacket.type == "chat") {
                if (currPacket.data.type == "message")
                    handleMessage(currPacket);
                else if (currPacket.data.type == "markSeen")
                    handleMsgMarkSeen(currPacket);
                else if (currPacket.data.type == "block")
                    handleBlock(currPacket);
                else if (currPacket.data.type == "gameInvite")
                    handleGameInvite(currPacket);
                else if (currPacket.data.type == "inviteAccepted" || currPacket.data.type == "inviteDeclined")
                    handleGameInviteRes(currPacket);
            }
            else if (currPacket.type == "notification") {
                if (currPacket.data.type == "markSeen")
                    handleNotifMarkSeen(currPacket);
                else if (currPacket.data.type == "friendReq")
                    checkNotificationFriend(currPacket);
                else if (currPacket.data.type == "friendAccept")
                    checkNotificationFriend(currPacket);
            }
            else if (currPacket.type == "logNotif")
                handleLogNotif(currPacket);
            else if (currPacket.type == "gameEvent")
                notifyTournamentStart(currPacket);
            else if (currPacket.type == "NotifyChange")
                notifyMembersChange(currPacket);
        }
        catch (error) {
...
function checkValidPacket(packet, userId) {
    if (packet.type == "onlineStatus" || (packet.type == "chat" && packet.data.message.length > 1000))
        return false;
    else if (packet.type == "logNotif" || packet.type == "NotifyChange")
        return true;
        ...
        if (!checkAdmin || checkAdmin.status != "ongoing" || JSON.parse(checkAdmin.players).length != 4)
            return false;
        return true;
    }
    if (packet.data.sender_id != userId || packet.data.sender_id == packet.data.recipient_id)
        return false;
    ...
    if ((!checkFriend && packet.data.type != "friendReq" && packet.data.type != "friendAccept" && packet.data.type != "block") || // send friendReq packet if not friend
...
function broadcastToFriends(userId, status) {
    const friendsDB = app.db.prepare("SELECT friends FROM players WHERE id = ?").get(userId)?.friends;
    if (!friendsDB)
        return;
    const friends = JSON.parse(friendsDB);
    const packet = {
        type: "onlineStatus",
        data: {
            type: "singleFriend",
...
export const chatService = {
    ...
    handler: (connection, req, res) => {
        ...
        connection.on("message", (message) => {
            try {
                const msgPacket = JSON.parse(message.toString());
