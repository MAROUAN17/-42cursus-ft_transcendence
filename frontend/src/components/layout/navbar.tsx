...
const Navbar = () => {
    ...
    if (newNotif.type == "message") {
      ...
      setNotifications((prev) => {
        return prev.map((notif) => {
          return notif.sender_id == packet.data.recipient_id
            ? { ...notif, unreadCount: 0 }
            : notif;
        });
      });
    } else if (
      newNotif.type == "friendReq" ||
      newNotif.type == "friendAccept"
    ) {
  ...
  function markNotifSeen(notification: notificationPacket) {
    ...
    setNotifications((prev) =>
      prev.map((notif) =>
        notif.id == notification.id ? { ...notif, unreadCount: 0 } : notif
      )
    );
  }
        ...
        <div className="flex flex-row gap-3">
          ...
          <div className="relative">
            <div
              ...
              className="p-3 gap-1 flex items-center hover:bg-neon/[20%] rounded-xl"
            >
            ...
            {isUserOptionOpen ? (
              <div className="absolute overflow-hidden right-0 mt-2 w-fit z-10 bg-[#1f085f] border-2 border-neon/10 rounded-lg shadow-[0_0px_1px_rgba(0,0,0,0.25)] shadow-neon">
                <ul>
                  <li
                    className="text-red-500 flex items-center hover:bg-compBg/30 gap-2 justify-center py-2 px-4"
