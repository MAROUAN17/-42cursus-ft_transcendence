...
function Login() {
  ...
  class WebsocketConnection {
    ...
    pageMethods: Map<string, () => void>;
    constructor() {
      this.ws = null;
      this.currPage = null;
      this.pageMethods = new Map<string, () => void>();
    }
    connect() {
      if (this.ws) return;
      this.ws = new WebSocket("wss://localhost:5000/send-message");
      this.ws.onopen = () => {
      ...
      this.ws.onmessage = (event) => {
        ...
        if (this.currPage && this.pageMethods.has(this.currPage)) {
          const method: (() => void) | undefined = this.pageMethods.get(
            this.currPage
          );
          if (method) method();
        }
    ...
    addPageMethod(method: () => void) {
      if (
        this.currPage &&
        this.pageMethods.has(this.currPage) &&
        this.pageMethods.get(this.currPage) == method
      )
        return;
      if (this.currPage) this.pageMethods.set(this.currPage, method);
    }
  }

  const connectWebsocket = () => {
    window.ws = new WebsocketConnection()
  };
        ...
        <form onSubmit={handleForm}>
          ...
          <div className="my-24 space-y-12">
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                type="submit"
                className="px-12 py-4 rounded-xl text-white bg-neon font-bold shadow-neon shadow-[0_10px_40px_rgba(0,0,0,0.1)]"
              >
                Login
              </button>
            </div>
